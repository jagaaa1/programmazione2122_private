...
public class Queens {
	...
	public static ChessboardView visualize(SList<Board> lst) {
		ChessboardView gui;
		Board brd;
		int i = 0;
		while (i < lst.length()) {
			brd = lst.listRef(i);
			gui = new ChessboardView(brd.size());
			gui.setQueens(brd.arrangement());
			i++;
		}
		return null;
	}

	public static ChessboardView graphicSolutions(int n) {
		visualize(listOfAllSolutions(n));
		return null;
	}
} // class Queens
